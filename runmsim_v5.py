...
integrations = 5 #Number of integrations
...
start_index = 9 #Start index for dither pattern
...
python -W ignore runmsim_v5.py [input file]

Where [input file] can be a NEMESISCubes directory, generated from
NEMESIS and processed into 24 .fits files (ending with either wave or radJy)
using the idl script made by Leigh Fletcher on ALICE. This NEMESIS data needs
to be converted into a .fits cube for MIRISim processing The input can also be
the already generated .fits cube.
...
def run_sim(array):
	...
	if newdisperser=='SHORT':
		newConfigPath='MRS_1SHORT'
	if newdisperser=='MEDIUM':
		time.sleep(2)
		newConfigPath = 'MRS_1MEDIUM'
	if newdisperser=='LONG':
		time.sleep(4)
		newConfigPath = 'MRS_1LONG'
...
if not os.path.exists(output_dir):
	...
	print('Output directory: {} already exists\nDelete or move this directory before running MIRISim\n'.format(output_dir))
