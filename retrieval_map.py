...
file = 'stage3/Level3_ch2-short_s3d.fits'
map_dir = 'RAdec/latlongmu/ch2-short'
outfile = 'aerosol_spectra'
...
image_trimmed = image[idx_start:idx_stop]
wave_trimmed = wavelist[idx_start:idx_stop]
...
#Removes o values from array and reshapes
for i in range(length3):
	for j in range(dy):
		for ii in range(dx):
			if image_trimmed[i][j][ii] == 0:
...
for i in range(length4):
	...
	st = np.transpose(sa)
	np.savetxt(outfile + '/{}_flux.txt'.format(i+1),st,fmt='%.6e',delimiter='	')


longlatemi_map(map_dir)
