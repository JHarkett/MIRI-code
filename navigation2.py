...
Usage
	If not done already, create the jwst_navs environment
        ...
        Input observation parameters below
...
class WcsSpiceNav:
	...
	def image2body(self, x, y):
		...
		return lon*dpr, lat*dpr, pglat*dpr, mu, muz, az
...
lone, latc, latg, mu, muz, az = nav.image2body(x, y)
#print(slat, slon)
#print(x, y)
#print(latc, lone)
...
for i,f in enumerate(files):
	...
	for y in range(ny):
		...
		for x in range(nx):
			if math.sqrt((y-yc)**2 + (x-xc)**2) < re_pix*1.05:
				lon, lat, glat, mu, muz, az = nav.image2body(x, y)
	...
	hduout = fits.HDUList([primary_hdu, image_hdu, lat_hdu, lon_hdu, mu_hdu, muz_hdu, az_hdu])
	base, ext = f.split('.')
